...
popRange <- scenVars[["maxPop"]][1]-scenVars[["minPop"]][1]

startVals <- c(scenVars[["minPop"]][1],
               scenVars[["minPop"]][1]+round(popRange/4,0),
               scenVars[["maxPop"]][1]-round(popRange/4,0),
               scenVars[["maxPop"]][1]
)
...
for(i in 1:2){
  ...
  writeInputSetFile3(Population=startVals[i],scenVars,sf)
  ...
  #Get Result
  thisrow <- getResRowVeneer(scenVars,i,Population = startVals[i])
  if(i==1){
    resDF <- thisrow
  } else{
...
lastRoot <- getLinearRootTarget(scenVars,i)
x <- 3
i <- 3
...
while( phi > 0.00001){
  ...
  print(paste0("    Current Population Estimate = ",lastRoot))
  print(paste0("    Current Net Take Estimate = ",round(resDF$NetTake[nrow(resDF)],2)))
  
  writeInputSetFile3(Population = lastRoot,scenVars,sf)
  ...
  #Get Result
  thisrow <- getResRowVeneer(scenVars,i,Population = lastRoot)
  if(i==1){
    resDF <- thisrow
  } else{
    resDF <- rbind(resDF,thisrow)
  }
  lastRoot <- getLinearRootTarget(scenVars,x)  #getRoot(scenVars,x)
  if(lastRoot<scenVars[["minPop"]][1]||lastRoot>scenVars[["maxPop"]][1]){
    lastRoot <- sample(scenVars[["minPop"]][1]:scenVars[["maxPop"]][1],1)
...
finalRow <- getResRowVeneer(scenVars,i,Population = lastRoot)
resDF <- rbind(resDF,finalRow)

print(paste0("-------------SDL Optimisation Complete-------------"))
print(paste0("Optimum Population = ",lastRoot))
